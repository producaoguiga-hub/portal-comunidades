...
import { Heart, UserCheck, Calendar, Building2, MapPin } from 'lucide-react'
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const loadStats = async () => {
      ...
      const [comRes, funcRes, acaoRes, assocRes] = await Promise.all([
      ...
      for (const c of comRes.data ?? []) {
        stats[c.id] = { nome: c.nome, funcionarios: 0, acoes: 0, valor: 0, associacoes: 0 }
      }
      ...
      const arr = Object.values(stats)
        .filter(s => s.associacoes > 0 || s.funcionarios > 0 || s.acoes > 0)
        .sort((a, b) => (b.funcionarios + b.acoes) - (a.funcionarios + a.acoes))
                ...
                {communityStats.map((s, i) => (
                  <tr key={i} className="hover:bg-verde/5 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className="inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold bg-oceano text-white">{s.associacoes}</span>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <span className="inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold bg-laranja text-white">{s.funcionarios}</span>
                    </td>
                    <td className="px-4 py-3 text-center">
